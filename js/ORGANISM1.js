...
Organism1.prototype.update = function() {
    ...
    // EVOLVING //
    if (this.energy / this.settings.feedCap >= 0.8) {
        if(tombola.range(0,10) <= 1 && org2.length < 2){
            var area = 10 * scale;
...
Organism1.prototype.kill = function() {
    ...
    var area = 30 * scale;
    generateSpores(this.size * 0.22, this.position.x - area, this.position.y - area, this.position.x + area, this.position.y + area);
